...
# S3 Backend Log bucket
resource "aws_s3_bucket" "log_bucket" {
  bucket = "${var.project_name}-tfstate-log-bucket"

  tags = {
    Name    = "${var.project_name}-tfstate-log-bucket"
    Project = var.project_name
  }
}
...
resource "aws_sns_topic" "log_bucket" {
  name = "tfstate-log-bucket-notifications"
}
...
resource "aws_s3_bucket_notification" "log_bucket" {
  ...
  topic {
    topic_arn     = aws_sns_topic.log_bucket.arn
    events        = ["s3:ObjectCreated:*"]
    filter_prefix = "logs/"
  }
}

resource "aws_sns_topic" "tfstate" {
  name = "tfstate-notifications"
}
...
resource "aws_s3_bucket_notification" "tfstate" {
  ...
  topic {
    topic_arn     = aws_sns_topic.tfstate.arn
    events        = ["s3:ObjectCreated:*"]
    filter_prefix = ""
  }
}
